...
function AnimatedOrb() {
  ...
  return (
    <Sphere args={[1.1, 62, 62]} ref={meshRef}>
      <MeshDistortMaterial
        color="#8b5cf6"
        attach="material"
        distort={0.45}
  ...
  return (
    <div className="fixed bottom-6 right-6 z-45 no-print">
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute bottom-18 right-0 w-80 p-5 rounded-3xl glass-panel border border-brand-500/20 shadow-2xl space-y-3 bg-white/70 dark:bg-dark-350/70"
          >
            ...
            <p className="text-xs text-slate-650 dark:text-slate-350 leading-relaxed min-h-12">
              {TIPS[tipIndex]}
            </p>
      ...
      <button
        ...
        className="relative group w-14 h-14 rounded-full overflow-hidden bg-slate-950 dark:bg-black flex items-center justify-center text-white shadow-xl hover:shadow-brand-500/30 hover:scale-105 active:scale-95 transition duration-300 z-10"
      >
        {!reducedMotion ? (
          ...
          <div className="absolute inset-0 bg-gradient-to-tr from-brand-600 to-indigo-600" />
        )}
        <MessageSquare className="w-5 h-5 text-white z-10 pointer-events-none group-hover:rotate-12 transition-transform duration-300" />
        
        {/* Glowing Rings (Disabled if reducedMotion is active) */}
        {!reducedMotion && (
          <>
            <span className="absolute inset-0 rounded-full bg-brand-500/20 animate-ping pointer-events-none" style={{ animationDuration: '2.5s' }} />
            <span className="absolute -inset-1 rounded-full border border-brand-500/10 blur-sm animate-pulse pointer-events-none" />
          </>
        )}
      </button>
    </div>
