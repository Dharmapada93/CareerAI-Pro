...
export default function CoverLetter() {
  ...
  const fetchLetters = async () => {
    try {
      const data = await apiFetch("/cover-letter");
      setLetters(data || []);
    ...
    <div className="relative z-10 pb-16">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 items-start">
        ...
        <div className="xl:col-span-2 space-y-6 text-left">
          {/* Input Box */}
          <div className="glass-panel p-6 rounded-3xl bg-white/75 dark:bg-dark-350/75">
          ...
          <AnimatePresence mode="wait">
            {activeLetter && (
              <motion.div
                ...
                exit={{ opacity: 0, y: -15 }}
                className="glass-panel p-6 sm:p-8 rounded-3xl space-y-4 bg-white/75 dark:bg-dark-350/75"
              >
                <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800/40 pb-4">
                  <div>
                    <h3 className="text-base font-bold text-slate-850 dark:text-slate-150 flex items-center">
                  ...
                  <div className="flex items-center space-x-2">
                    <button
                      onClick={handleCopy}
                      className="p-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 transition flex items-center justify-center"
        ...
        <div className="glass-panel p-6 rounded-3xl text-left bg-white/75 dark:bg-dark-350/75">
          ...
          {historyLoading ? (
            <div className="flex items-center justify-center py-12">
              <Loader2 className="w-5 h-5 animate-spin text-slate-455" />
            </div>
          ) : letters.length === 0 ? (
