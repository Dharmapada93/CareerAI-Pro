...
export default function PortfolioBuilder() {
  ...
  const fetchPortfolio = async () => {
    try {
      ...
      if (data) {
        ...
        setContent(data.content || { heroTitle: "", heroSubtitle: "", aboutMe: "", projectSubtitles: {} });
      }

      // Fetch user's resumes to supply the AI builder
      const resumeData = await apiFetch("/resume");
      setResumesList(resumeData || []);
      setHasResumes(resumeData && resumeData.length > 0);
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
  ...
  const handleAIGenerate = async () => {
    ...
    try {
      ...
      console.error(err);
      setError(err?.message || "AI Generation failed. Check backend connectivity.");
    } finally {
      setAiLoading(false);
    }
  };
  ...
  <main class="container">
    ...
    <section class="skills">
      ...
      <div class="skills-grid">
        ${skills.map((s: any) => `<span class="skill-tag">${s}</span>`).join("\n        ")}
      </div>
    </section>
    ...
    <div className="relative z-10 pb-16">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="glass-panel p-6 sm:p-8 rounded-3xl space-y-6 bg-white/75 dark:bg-dark-350/75">
              ...
              {slug && (
                <div className="flex items-center space-x-2">
                  <Link
                    to={`/portfolio/${slug}`}
                    target="_blank"
                    className="px-3 py-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-850 dark:hover:bg-slate-800 text-xs font-bold text-slate-655 dark:text-slate-300 flex items-center"
                  >
                    <span>View Site</span>
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-850 dark:hover:bg-slate-800 text-xs font-bold text-slate-655 dark:text-slate-300 flex items-center"
                  >
                    {copied ? <Check className="w-3.5 h-3.5 text-emerald-500 mr-1" /> : <Link2 className="w-3.5 h-3.5 mr-1" />}
                    <span>Copy Link</span>
                  </button>
                </div>
              )}
            ...
            {error && (
              <div className="flex items-center space-x-2 p-3.5 bg-red-500/10 border border-red-500/20 text-red-500 rounded-2xl text-xs text-left">
            ...
            {success && (
              <div className="flex items-center space-x-2 p-3.5 bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-450 rounded-2xl text-xs text-left">
            ...
            <form onSubmit={handleSave} className="space-y-6 text-left">
                ...
                <div className="flex items-center space-x-6 pt-5">
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      className="rounded border-slate-300 text-brand-605 focus:ring-brand-505"
                    />
                  ...
                  <div>
                    <label className="inline-flex items-center space-x-2 text-xs font-bold text-slate-705 dark:text-slate-350">
                      <span>Theme:</span>
                      <select
                        value={theme}
              ...
              <div className="p-5 rounded-2xl bg-brand-500/[0.03] border border-brand-500/10 flex flex-col md:flex-row items-center justify-between gap-4">
                <div className="space-y-1 text-left">
                  <h4 className="text-xs font-bold text-brand-605 dark:text-brand-400 flex items-center">
                    <Sparkles className="w-3.5 h-3.5 mr-1" />
                    <span>AI Portfolio Generator</span>
                  </h4>
                  <p className="text-[10px] text-slate-450 dark:text-slate-555 leading-normal max-w-md">
              ...
              <div className="space-y-4 pt-4 border-t border-slate-100 dark:border-slate-800/40">
                <h4 className="text-sm font-bold text-slate-805 dark:text-slate-200 flex items-center">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="glass-panel p-6 rounded-3xl text-left bg-white/75 dark:bg-dark-350/75">
            ...
            {!content.heroTitle ? (
              ...
              <div className={`p-5 rounded-2xl border text-left space-y-4 ${
                ...
                <p className={`text-[11px] leading-relaxed line-clamp-4 ${theme === "dark" ? "text-slate-300" : "text-slate-600"}`}>
                  {content.aboutMe}
                </p>

                <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex justify-end">
                  <span className="text-[10px] font-bold text-indigo-505 flex items-center">
          ...
          {/* EXPORT ZIP CARD */}
          <div className="glass-panel p-6 rounded-3xl text-left space-y-4 bg-white/75 dark:bg-dark-350/75">
            ...
            <p className="text-[11px] leading-relaxed text-slate-450 dark:text-slate-550 font-medium">
                  ...
                  <button
                    ...
                    className={`text-[10px] font-bold py-2 rounded-xl border text-center transition capitalize ${
                      exportTheme === style.id
                        ...
                        : "bg-slate-100 border-slate-200 text-slate-600 dark:bg-slate-800/45 dark:border-slate-800 dark:text-slate-350 hover:bg-slate-200"
                    }`}
                  >
                    {style.name}
                  </button>
