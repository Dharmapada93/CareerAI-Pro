...
import ThreeDResumePreview from "../components/3d/ThreeDResumePreview";
...
export default function ResumeEditor() {
  ...
  const handleSave = async () => {
    ...
    setSaveLoading(true);
    try {
      await apiFetch(`/resume/${resumeId}`, {
        method: "PUT",
        body: resumeData,
      });
  ...
  const handleGenerateSummary = async () => {
    ...
    try {
      const data = await apiFetch("/resume/ai-summary", {
        method: "POST",
        body: { skills, experience: experiences },
      });
  ...
  const handleOpenBulletAI = async (expIndex: number, bulletIndex: number) => {
    ...
    try {
      const data = await apiFetch("/resume/ai-bullets", {
        method: "POST",
        body: {
  ...
  const handleAITailor = async () => {
    ...
    try {
      const data = await apiFetch("/resume/ai-tailor", {
        method: "POST",
        body: {
          resume_data: {
  ...
  const handleAITone = async () => {
    ...
    try {
      const data = await apiFetch("/resume/ai-tone", {
        method: "POST",
        body: { text: originalText, tone: selectedTone },
      });
  ...
  const handleAIGrammar = async (text: string, targetKey: string) => {
    ...
    try {
      const data = await apiFetch("/resume/ai-grammar", {
        method: "POST",
        body: { text },
      });
      setGrammarResults({
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between pb-6 mb-6 border-b border-slate-200/50 dark:border-slate-800/40 no-print">
        <div className="flex items-center space-x-4">
          <button
            ...
            className="p-2.5 rounded-xl border border-slate-200 dark:border-slate-800 hover:bg-slate-100 dark:hover:bg-slate-800/40 text-slate-550"
          >
        ...
        <div className="flex items-center space-x-3 mt-4 sm:mt-0">
          <button
            ...
            className="px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-800 hover:bg-slate-100 dark:hover:bg-slate-800/40 text-slate-700 dark:text-slate-300 text-sm font-semibold flex items-center justify-center space-x-1.5 transition"
          >
        ...
        <div className="space-y-4 no-print">
          ...
          <div className="glass-panel rounded-3xl overflow-hidden border border-brand-500/20">
            <button
              ...
              className="w-full flex items-center justify-between p-6 font-bold text-brand-600 dark:text-brand-400 hover:bg-slate-50/50 dark:hover:bg-slate-900/10 text-left bg-brand-500/[0.02]"
            >
            ...
            {activeSection === "ai-upgrades" && (
              <div className="p-6 border-t border-slate-100 dark:border-slate-800/40 space-y-6 text-left">
                {/* 1. Tailor Resume */}
                <div className="space-y-3">
                  ...
                  <div className="flex justify-end">
                    <button
                      ...
                      className="px-4 py-2 rounded-xl bg-slate-900 dark:bg-slate-800 text-white font-bold text-xs flex items-center space-x-1.5 transition disabled:opacity-50"
                    >
                ...
                <div className="space-y-3 pt-4 border-t border-slate-100 dark:border-slate-800/40">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div>
                      ...
                      <select
                        ...
                        className="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-800/40 bg-transparent text-xs text-slate-900 dark:text-white"
                      >
                  ...
                  <div className="flex justify-end">
                    <button
                      ...
                      className="px-4 py-2 rounded-xl bg-slate-900 dark:bg-slate-800 text-white font-bold text-xs flex items-center space-x-1.5 transition"
                    >
                    ...
                    <div className="p-4 rounded-2xl bg-brand-500/[0.03] border border-brand-500/10 space-y-3">
                      ...
                      {grammarResults.errorsFound.length === 0 ? (
                        ...
                        <div className="space-y-1">
                          ...
                          <ul className="list-disc pl-4 text-[11px] text-slate-500 space-y-0.5 text-left">
                      ...
                      <div className="pt-2 border-t border-slate-100 dark:border-slate-800/40 space-y-2 text-left">
                        <p className="text-xs font-bold text-slate-700 dark:text-slate-350">Proposed Correction:</p>
                        <p className="p-3 bg-slate-100 dark:bg-slate-900/60 rounded-xl text-xs leading-relaxed italic text-slate-600 dark:text-slate-350">
                ...
                <div className="text-left">
                  <div className="flex justify-between items-center mb-1.5">
                    ...
                    <button
                      ...
                      className="text-xs text-brand-600 dark:text-brand-400 font-bold hover:underline flex items-center space-x-1"
                    >
                      {aiSummaryLoading ? (
          ...
          <div className="glass-panel rounded-3xl overflow-hidden">
            ...
            {activeSection === "experience" && (
              <div className="p-6 border-t border-slate-100 dark:border-slate-800/40 space-y-6">
                {resumeData.experience.map((exp, expIdx) => (
                  <div key={expIdx} className="p-5 rounded-2xl bg-slate-50/50 dark:bg-slate-900/20 border border-slate-200/30 dark:border-slate-800/30 space-y-4 relative text-left">
                    <button
                        ...
                        {exp.description.map((bullet, bullIdx) => (
                          <div key={bullIdx} className="flex items-start space-x-2">
                            ...
                            <button
                              ...
                              className="p-2 rounded-xl bg-purple-500/10 text-purple-650 dark:text-purple-400 hover:bg-purple-600 hover:text-white transition"
            ...
            {activeSection === "education" && (
              <div className="p-6 border-t border-slate-100 dark:border-slate-800/40 space-y-6">
                {resumeData.education.map((edu, eduIdx) => (
                  <div key={eduIdx} className="p-5 rounded-2xl bg-slate-50/50 dark:bg-slate-900/20 border border-slate-200/30 dark:border-slate-800/30 space-y-4 relative text-left">
                    <button
            ...
            {activeSection === "projects" && (
              <div className="p-6 border-t border-slate-100 dark:border-slate-800/40 space-y-6">
                {resumeData.projects.map((proj, projIdx) => (
                  <div key={projIdx} className="p-5 rounded-2xl bg-slate-50/50 dark:bg-slate-900/20 border border-slate-200/30 dark:border-slate-800/30 space-y-4 relative text-left">
                    <button
          ...
          <div className="glass-panel rounded-3xl overflow-hidden">
            ...
            {activeSection === "skills" && (
              <div className="p-6 border-t border-slate-100 dark:border-slate-800/40 space-y-4 text-left">
                <div className="flex space-x-2">
                  ...
                  <button
                    ...
                    className="px-4.5 py-2.5 rounded-xl bg-slate-900 dark:bg-slate-850 dark:hover:bg-slate-800 text-white text-xs font-semibold hover:bg-slate-800 transition"
                  >
                    Add
                  </button>
                </div>
                ...
                <div className="flex flex-wrap gap-2 pt-2">
                  {resumeData.skills.map((skill, idx) => (
                    <span
                      key={idx}
                      className="inline-flex items-center space-x-1 px-3 py-1 rounded-full bg-brand-500/10 text-brand-600 dark:text-brand-400 text-xs font-semibold"
                    >
                      <span>{skill}</span>
                      <button
                        type="button"
            ...
            {activeSection === "certificates" && (
              <div className="p-6 border-t border-slate-100 dark:border-slate-800/40 space-y-6">
                {resumeData.certificates.map((cert, certIdx) => (
                  <div key={certIdx} className="p-5 rounded-2xl bg-slate-50/50 dark:bg-slate-900/20 border border-slate-200/30 dark:border-slate-800/30 space-y-4 relative text-left">
                    <button
          ...
          <div className="flex items-center justify-between mb-4 no-print bg-white/70 dark:bg-slate-900/60 p-3 rounded-2xl border border-slate-200/50 dark:border-slate-800/40 backdrop-blur-md">
            ...
            <div className="flex items-center space-x-2.5">
              <button
                ...
                className={`text-xs px-3 py-1.5 rounded-xl font-bold transition flex items-center space-x-1 ${
                  threeDMode
          ...
          <div className="w-full aspect-[1/1.4142] bg-white border border-slate-200 dark:border-transparent text-slate-800 p-8 sm:p-10 shadow-2xl rounded-2xl md:rounded-3xl overflow-y-auto max-h-[85vh] print-page relative text-left">
            {threeDMode ? (
              <div className="flex flex-col items-center justify-center h-full py-6 text-center space-y-4">
                <ThreeDResumePreview />
                <div>
                  <p className="text-sm font-bold text-slate-800 dark:text-slate-650">3D Interactive Canvas Active</p>
                  <p className="text-xs text-slate-400 mt-1">
                    Hover and drag the card. Toggle off 3D Mode to edit details or export to PDF.
                  </p>
                </div>
              </div>
            ) : (
              ...
              <div className="space-y-6">
                ...
                <div className="border-b-4 border-brand-500 pb-5">
                  ...
                  <div className="flex flex-wrap gap-x-4 gap-y-1.5 mt-2.5 text-xs text-slate-500 font-semibold">
                    ...
                    {resumeData.personalInfo.website && (
                      <span className="text-brand-605 underline">{resumeData.personalInfo.website}</span>
                    )}
                ...
                {resumeData.personalInfo.summary && (
                  <div className="space-y-1">
                    ...
                    <p className="text-xs leading-relaxed text-slate-600">{resumeData.personalInfo.summary}</p>
                  </div>
                )}
                        ...
                        <div key={idx} className="space-y-1.5">
                          <div className="flex justify-between items-start text-xs">
                            <div>
                              ...
                              <span className="font-bold text-brand-605">{exp.company}</span>
                            </div>
                          ...
                          <ul className="list-disc list-inside space-y-1 pl-1">
                            {exp.description.map((bullet, bIdx) => (
                              <li key={bIdx} className="text-xs text-slate-600 leading-relaxed list-item">
                                {bullet || "Draft point..."}
                              </li>
                            ))}
                          </ul>
                      ...
                      {resumeData.projects.map((proj, idx) => (
                        <div key={idx} className="space-y-0.5 text-xs">
                          <div className="flex justify-between">
                            ...
                            {proj.url && <span className="text-[10px] text-brand-600 underline">{proj.url}</span>}
                          </div>
            ...
            {resumeData.templateId === "minimal" && (
              <div className="space-y-6 text-slate-950 font-serif">
                ...
                <div className="text-center space-y-1">
                  ...
                  <div className="flex justify-center flex-wrap gap-x-4 text-[10px] uppercase font-sans text-slate-550 tracking-wider">
                    {resumeData.personalInfo.email && <span>{resumeData.personalInfo.email}</span>}
                    {resumeData.personalInfo.phone && <span>{resumeData.personalInfo.phone}</span>}
              ...
              <div className="space-y-5">
                ...
                {resumeData.personalInfo.summary && (
                  <div className="space-y-1 text-xs">
                    ...
                    <p className="leading-relaxed text-slate-600">{resumeData.personalInfo.summary}</p>
                  </div>
                )}
        ...
        {bulletModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 no-print">
            ...
            <motion.div
              ...
              className="bg-white dark:bg-dark-400 p-6 sm:p-8 rounded-3xl w-full max-w-xl shadow-2xl border border-slate-200/50 dark:border-slate-800/60 z-10 relative overflow-hidden"
            >
              <button
                onClick={() => setBulletModalOpen(false)}
              ...
              <div className="mt-6 space-y-4">
                <div>
                  ...
                  <p className="p-3 bg-slate-50 dark:bg-slate-900/40 rounded-xl border border-slate-200/40 dark:border-slate-800/20 text-xs italic text-slate-650 dark:text-slate-400">
                    &ldquo;{customBulletInput}&rdquo;
                  </p>
                </div>

                <div>
                  <label className="block text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-2 text-left">
                    ...
                    <div className="space-y-3.5">
                      {bulletOptions.map((opt, oIdx) => (
                        <button
                          ...
                          className="w-full text-left p-4 rounded-xl border border-slate-200/50 dark:border-slate-800/60 hover:border-brand-500 hover:bg-brand-500/[0.02] dark:hover:bg-brand-500/[0.03] transition flex items-start space-x-3 group"
                        >
                          <span className="p-1 rounded-full bg-brand-500/10 text-brand-600 dark:text-brand-400 mt-0.5 flex-shrink-0">
                            <Check className="w-3 h-3" />
                          </span>
                          <span className="text-xs font-medium text-slate-700 dark:text-slate-200 group-hover:text-brand-700 dark:group-hover:text-brand-400 transition">
                            {opt}
                          </span>
                        </button>
                      ))}
        ...
        {diffData && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 no-print">
            ...
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 10 }}
              className="bg-white dark:bg-dark-400 p-6 sm:p-8 rounded-3xl w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl border border-slate-200/50 dark:border-slate-800/60 z-10 relative"
            >
              <button
                onClick={() => setDiffData(null)}
                className="absolute top-4 right-4 p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800/50 text-slate-400"
              ...
              <div className="mt-6 space-y-6 text-left">
                ...
                {diffData.type === "tailor" && (
                  <div className="space-y-6">
                    ...
                    {diffData.original.summary !== diffData.proposed.summary && (
                      <div className="space-y-2">
                        <h3 className="text-xs font-bold text-slate-505 uppercase tracking-wider">Professional Summary</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="p-4 rounded-2xl bg-red-500/5 border border-red-500/10 text-xs text-slate-600 dark:text-slate-400">
                            <span className="font-bold text-red-500 block mb-1">Original Summary</span>
                          ...
                          <div className="p-4 rounded-2xl bg-emerald-500/5 border border-emerald-500/10 text-xs text-slate-700 dark:text-slate-200">
                            <span className="font-bold text-emerald-500 block mb-1">Proposed Tailored Summary</span>
                    ...
                    {/* Skills Comparison */}
                    <div className="space-y-2">
                      <h3 className="text-xs font-bold text-slate-505 uppercase tracking-wider">Tailored Skills List</h3>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="p-4 rounded-2xl bg-red-500/5 border border-red-500/10 text-xs text-slate-600 dark:text-slate-400">
                          <span className="font-bold text-red-500 block mb-2">Original Skills</span>
                        ...
                        <div className="p-4 rounded-2xl bg-emerald-500/5 border border-emerald-500/10 text-xs text-slate-700 dark:text-slate-200">
                          <span className="font-bold text-emerald-500 block mb-2">Proposed Skills (Tailored/Added)</span>
                          <div className="flex flex-wrap gap-1.5">
                    ...
                    {/* Projects Comparison */}
                    <div className="space-y-3">
                      <h3 className="text-xs font-bold text-slate-505 uppercase tracking-wider">Tailored Projects</h3>
                      <div className="space-y-4">
                        {diffData.proposed.projects.map((proj: any, pIdx: number) => {
                          ...
                          return (
                            <div key={pIdx} className="space-y-2 border-t border-slate-100 dark:border-slate-800/40 pt-3">
                              <span className="text-xs font-bold text-slate-700 dark:text-slate-300">{proj.title}</span>
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="p-4 rounded-2xl bg-red-500/5 border border-red-500/10 text-xs text-slate-600 dark:text-slate-400">
                                  <span className="font-bold text-red-500 block mb-1">Original Description</span>
                  ...
                  <div className="space-y-4 text-left">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="p-5 rounded-2xl bg-red-500/5 border border-red-500/10 text-xs text-slate-600 dark:text-slate-400">
                        <span className="font-bold text-red-500 block mb-2">Original Version</span>
                        <p className="leading-relaxed italic">&ldquo;{diffData.original}&rdquo;</p>
                      </div>
                      <div className="p-5 rounded-2xl bg-emerald-500/5 border border-emerald-500/10 text-xs text-slate-700 dark:text-slate-205">
                        <span className="font-bold text-emerald-500 block mb-2">
                ...
                <div className="flex justify-end space-x-3 pt-6 border-t border-slate-100 dark:border-slate-800/40">
                  <button
                    onClick={() => setDiffData(null)}
                    className="px-5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-900/40 text-xs font-bold transition"
                  >
                    Reject Changes
                  </button>
                  <button
