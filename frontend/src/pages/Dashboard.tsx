...
export default function Dashboard() {
  ...
  const { data: interviews = [], isLoading: isInterviewsLoading } = useQuery({
    queryKey: ["interviews"],
    queryFn: () => apiFetch("/interview"),
  });

  // Fetch ATS reports
  const { data: atsReports = [], isLoading: isAtsLoading } = useQuery({
    queryKey: ["atsReports"],
    queryFn: () => apiFetch("/ats"),
  });

  if (isProfileLoading || isResumesLoading || isInterviewsLoading || isAtsLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] space-y-4">
        <Loader2 className="w-8 h-8 animate-spin text-brand-650" />
        <p className="text-sm text-slate-500 dark:text-slate-400">Assembling dashboard panels...</p>
      </div>
    );
  }

  const resumeCount = profileData?.stats?.resumeCount || resumes.length || 0;
  const highestAts = profileData?.stats?.highestAts || (atsReports.length > 0 ? Math.max(...atsReports.map((r: any) => r.score)) : 0);
  ...
  const atsHistory = atsReports.slice(0, 10).map((r: any) => ({
    date: new Date(r.createdAt).toLocaleDateString("en-US", { month: "short", day: "numeric" }),
    score: r.score,
  })).reverse(); // order chronological for chart

  // Get recent 3 resumes
  const recentResumes = resumes.slice(0, 3);

  // Compile recent activities
  const activities: Array<{ id: string; type: string; title: string; subtitle: string; time: string; rawTime: number }> = [];
  
  resumes.slice(0, 3).forEach((r: any) => {
    activities.push({
      id: r._id,
      type: "resume",
      title: `Updated resume "${r.title}"`,
      subtitle: `${r.skills?.length || 0} skills • ${r.experience?.length || 0} experiences`,
      time: new Date(r.updatedAt).toLocaleDateString(),
      rawTime: new Date(r.updatedAt).getTime(),
    });
  });

  interviews.slice(0, 2).forEach((i: any) => {
    activities.push({
      id: i._id,
      type: "interview",
      ...
      rawTime: new Date(i.updatedAt).getTime(),
    });
  });
    ...
    <div className="relative z-10 pb-12 text-left">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div className="glass-panel p-6 rounded-3xl flex items-center justify-between bg-white/70 dark:bg-dark-350/70">
          ...
          <div className="p-3.5 rounded-2xl bg-brand-500/10 text-brand-650 dark:text-brand-400">
        ...
        <div className="glass-panel p-6 rounded-3xl flex items-center justify-between bg-white/70 dark:bg-dark-350/70">
          ...
          <div className="p-3.5 rounded-2xl bg-emerald-500/10 text-emerald-600 dark:text-emerald-450">
        ...
        <div className="glass-panel p-6 rounded-3xl flex items-center justify-between bg-white/70 dark:bg-dark-350/70">
          <div className="space-y-1">
            ...
            <Link to="/pricing" className="text-xs text-purple-500 hover:text-purple-400 font-medium flex items-center group">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 glass-panel p-6 rounded-3xl bg-white/70 dark:bg-dark-350/70">
              ...
              {recentResumes.map((r: any) => (
                ...
                >
                  <div className="flex items-center space-x-3">
                    <div className="p-2.5 rounded-xl bg-brand-500/10 text-brand-650 dark:text-brand-400">
        ...
        <div className="glass-panel p-6 rounded-3xl bg-white/70 dark:bg-dark-350/70">
