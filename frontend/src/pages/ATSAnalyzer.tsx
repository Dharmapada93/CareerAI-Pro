...
export default function ATSAnalyzer() {
  ...
  const handleAnalyze = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      setError(err?.message || "Failed to run ATS scanner. Please try again.");
    } finally {
      setLoading(false);
    }
  };
          ...
          <AnimatePresence mode="wait">
            {activeReport && (
              ...
              >
                ...
                <div className="glass-panel p-6 rounded-3xl flex flex-col md:flex-row items-center gap-6 justify-between overflow-hidden relative bg-white/75 dark:bg-dark-350/75">
                  {/* Glowing background */}
                  ...
                  <div className="flex flex-wrap gap-2 justify-center">
                    ...
                    <span className="text-[10px] font-semibold px-3 py-1 rounded-full bg-slate-150 text-slate-600 dark:bg-slate-800 dark:text-slate-400">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {/* Missing Keywords */}
                  <div className="glass-panel p-6 rounded-3xl space-y-3 bg-white/75 dark:bg-dark-350/75">
                    ...
                    <div className="flex flex-wrap gap-1.5 pt-2">
                      {activeReport.missingKeywords?.map((kw, idx) => (
                        <span
                          key={idx}
                          className="px-2.5 py-1 rounded-lg bg-red-500/10 border border-red-500/15 text-red-655 dark:text-red-400 text-xs font-semibold"
                        >
                          {kw}
                        </span>
                      ))}
                  ...
                  <div className="glass-panel p-6 rounded-3xl space-y-3 bg-white/75 dark:bg-dark-350/75">
                    ...
                    <ul className="space-y-2 pt-1">
                      {activeReport.formattingSuggestions?.map((sug, idx) => (
                        <li key={idx} className="text-xs text-slate-600 leading-relaxed flex items-start space-x-2 dark:text-slate-300">
        ...
        <div className="glass-panel p-6 rounded-3xl text-left bg-white/75 dark:bg-dark-350/75">
          ...
          {historyLoading ? (
            <div className="flex items-center justify-center py-12">
              <Loader2 className="w-5 h-5 animate-spin text-slate-405" />
            </div>
          ) : reports.length === 0 ? (
