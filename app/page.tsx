...
export default function LandingPage() {
  ...
  const coreFeatures = [
    ...
    },
    {
      ...
      color: "from-indigo-550 to-indigo-650",
    },
    {
      icon: MessageSquare,
  ...
  return (
    <div className="relative z-10 flex flex-col items-center">
      {/* SECTION 1: HERO */}
      <section className="min-h-[85vh] flex flex-col justify-center items-center px-4 max-w-5xl text-center space-y-8 relative overflow-hidden py-16">
        {/* Glow filters */}
        ...
        <motion.div
          ...
          className="space-y-4"
        >
          <span className="inline-flex items-center space-x-1.5 px-3.5 py-1.5 rounded-full bg-brand-500/10 text-brand-650 dark:text-brand-400 text-xs font-bold shadow-sm">
            <Sparkles className="w-4.5 h-4.5 text-brand-500 animate-pulse" />
            <span>Next-Gen Career SaaS for Freshers</span>
          </span>

          <h1 className="text-4xl sm:text-6xl font-black tracking-tight leading-none text-slate-805 dark:text-white max-w-4xl mx-auto">
        ...
        >
          {session ? (
            ...
            <>
              ...
              <Link
                href="/pricing"
                className="w-full sm:w-auto px-6 py-3.5 rounded-2xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-805 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-350 text-sm font-semibold transition"
              >
                View Pricing
              </Link>
            </>
      ...
      <section className="w-full max-w-6xl px-6 py-16 text-center">
        <h2 className="text-2xl sm:text-3xl font-extrabold text-slate-805 dark:text-white mb-10">
      ...
      <section id="features" className="w-full max-w-6xl px-6 py-16 text-center">
        <h2 className="text-2xl sm:text-3xl font-extrabold text-slate-805 dark:text-white mb-2">
          ...
          {coreFeatures.map((feat, idx) => {
            ...
            return (
              ...
              >
                ...
                <h3 className="text-base font-extrabold text-slate-850 dark:text-white group-hover:text-brand-500 transition">
                  {feat.title}
                </h3>
            ...
            >
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <h4 className="font-extrabold text-base text-slate-800 dark:text-slate-205">{tpl.name}</h4>
                  {tpl.popular && (
          ...
          {faqs.map((faq, idx) => (
            ...
            >
              ...
              >
                <span>{faq.q}</span>
                <ChevronDown
                  className={`w-4 h-4 text-slate-450 transition-transform ${activeFaq === idx ? "rotate-185" : ""}`}
                />
              </button>
              {activeFaq === idx && (
