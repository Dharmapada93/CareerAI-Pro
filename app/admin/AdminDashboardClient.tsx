...
export function AdminDashboardClient({
  ...
  // Loading indicator for updating subscription
  ...
  const handleUpdateSubscription = async (userId: string, newTier: "Free" | "Pro" | "Enterprise") => {
    ...
    try {
      ...
      if (!res.ok) {
        setActionError(data.error || "Failed to update subscription tier.");
      } else {
        // Update local state
      ...
      console.error(err);
      setActionError("Failed to communicate with subscription handler API.");
    } finally {
  ...
  const handleResolveFeedback = async (feedbackId: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const data = await res.json();
        setActionError(data.error || "Failed to resolve support report.");
      }
    } catch (err) {
      console.error(err);
      setActionError("Failed to communicate with support resolution API.");
    }
  };
            ...
            <button
              ...
              className={`flex items-center space-x-2 px-4.5 py-2.5 rounded-2xl text-xs font-bold transition-all ${
                activeTab === tab.id
                  ...
                  : "bg-slate-100 hover:bg-slate-205 text-slate-650 dark:bg-slate-800/45 dark:text-slate-350 dark:hover:bg-slate-800"
              }`}
            >
        ...
        <AnimatePresence mode="wait">
              ...
              <div className="glass-panel rounded-3xl overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-xs border-collapse">
                    <thead>
                      <tr className="text-slate-400 dark:text-slate-550 uppercase tracking-wider border-b border-slate-100 dark:border-slate-805">
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800/40">
                      {filteredUsers.map((usr) => (
                        <tr key={usr._id} className="text-slate-700 dark:text-slate-205 hover:bg-slate-500/[0.01]">
                          <td className="p-4 font-bold text-slate-850 dark:text-slate-200 flex items-center space-x-2">
                            <div className="w-8 h-8 rounded-full bg-brand-500/10 text-brand-605 flex items-center justify-center font-extrabold uppercase text-xs">
                            ...
                            <div className="inline-flex items-center space-x-1.5">
                              {subUpdatingId === usr._id ? (
                                ...
                                <select
                                  ...
                                  className="px-2 py-1 bg-slate-50 dark:bg-slate-850 border border-slate-200 dark:border-slate-800 rounded-lg text-[10px] font-bold"
                                >
                      ...
                      {filteredUsers.length === 0 && (
                        <tr>
                          <td colSpan={5} className="p-8 text-center text-slate-450">No registrations match filter.</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
            ...
            >
              <div className="glass-panel p-6 sm:p-8 rounded-3xl">
                ...
                <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-2">
                  {logs.map((log) => (
                    <div key={log._id} className="p-4 rounded-2xl bg-slate-50/50 dark:bg-slate-900/40 border border-slate-200/40 dark:border-slate-800/30 flex justify-between gap-4 text-xs">
                      <div className="space-y-1">
                        <div className="flex items-center space-x-2 flex-wrap gap-y-1">
                          <span className="px-2 py-0.5 rounded bg-brand-500/10 text-brand-650 dark:text-brand-405 font-bold text-[9px]">
                            {log.action}
                          </span>
                          {log.userId && (
                        ...
                        <p className="text-[11px] text-slate-605 dark:text-slate-300 font-medium">
                          {log.details}
                        </p>
                      </div>
                  ...
                  {logs.length === 0 && (
                    <p className="text-xs text-slate-450 py-12 text-center">No active security logs found.</p>
                  )}
                </div>
              </div>
            </motion.div>
          ...
          {activeTab === "feedback" && (
            ...
            >
              <AnimatePresence>
                {feedback.map((item) => (
                  <motion.div
                    ...
                    className="glass-panel p-5 rounded-2xl flex flex-col justify-between space-y-4 text-left relative"
                  >
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-slate-650 dark:text-slate-250 leading-relaxed pr-2 italic">
                        &ldquo;{item.message}&rdquo;
                      </p>
                    </div>

                    <div className="pt-3 border-t border-slate-100 dark:border-slate-805 flex justify-between items-center text-[10px] font-semibold text-slate-450">
              ...
              {feedback.length === 0 && (
                <div className="md:col-span-2 glass-panel p-12 text-center rounded-3xl text-slate-400 py-24 space-y-2">
                  <MessageSquare className="w-8 h-8 mx-auto text-slate-350 dark:text-slate-700" />
