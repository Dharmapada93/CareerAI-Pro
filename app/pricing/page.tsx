  ...
  const handleUpgrade = async (planName: "Free" | "Pro" | "Enterprise") => {
    ...
    try {
      const res = await fetch("/api/profile", {
        ...
        body: JSON.stringify({ subscription: planName }),
      });
      ...
      if (res.ok) {
        ...
        setErrorMsg("Failed to complete mock upgrade. Try again.");
      }
    } catch (err) {
      console.error(err);
      setErrorMsg("An unexpected error occurred during subscription simulation.");
    } finally {
  ...
  return (
    <div className="relative z-10 py-16 px-4 max-w-7xl mx-auto text-center">
      ...
      <div className="space-y-4 mb-10 max-w-3xl mx-auto">
        <span className="text-[10px] font-bold uppercase tracking-widest bg-brand-500/10 text-brand-650 dark:text-brand-400 px-3 py-1 rounded-full">
          Simple Subscription Model
        </span>
        <h1 className="text-4xl sm:text-5xl font-extrabold tracking-tight text-slate-805 dark:text-white">
          SaaS Pricing Plans
        </h1>
          ...
          <div className="relative p-1 bg-slate-100 dark:bg-slate-900 border border-slate-200/50 dark:border-slate-800/40 rounded-2xl flex items-center space-x-1">
            <button
              ...
              className={`text-xs font-bold px-4 py-2 rounded-xl transition ${
                billingPeriod === "monthly" ? "bg-white dark:bg-slate-800 text-brand-600 dark:text-white shadow-sm" : "text-slate-450 dark:text-slate-500"
              }`}
            >
              Monthly
            </button>
            <button
              ...
              className={`text-xs font-bold px-4 py-2 rounded-xl transition flex items-center space-x-1 ${
                billingPeriod === "annually" ? "bg-white dark:bg-slate-800 text-brand-600 dark:text-white shadow-sm" : "text-slate-450 dark:text-slate-500"
              }`}
            >
              <span>Annually</span>
      ...
      {successMsg && (
        <div className="mb-8 max-w-md mx-auto flex items-center space-x-2 p-4 bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-450 rounded-2xl text-sm justify-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-start mb-16 text-left">
        {plans.map((p) => {
          const isUserPlan = session?.user?.subscription === p.name;
            ...
            >
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-xs text-slate-400 dark:text-slate-500 mt-1.5 min-h-[32px]">{p.desc}</p>
                </div>
              ...
              <div className="mt-8 pt-4 border-t border-slate-100 dark:border-slate-800/40">
                {isUserPlan ? (
                  <button
                    disabled
                    className="w-full py-3 px-4 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-450 dark:text-slate-550 font-bold text-xs cursor-default flex items-center justify-center space-x-1.5"
                  >
                  ...
                  <button
                    ...
                    className={`w-full py-3 px-4 rounded-xl font-bold text-xs flex items-center justify-center space-x-1.5 transition ${
                      p.popular
                        ...
                        : "bg-slate-100 hover:bg-slate-200 dark:bg-slate-805 dark:hover:bg-slate-800 text-slate-750 dark:text-slate-300"
                    }`}
                  >
