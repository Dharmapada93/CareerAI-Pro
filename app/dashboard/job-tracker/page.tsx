...
export default function JobTrackerPage() {
  ...
  return (
    <div className="relative z-10 pb-16 max-w-7xl mx-auto px-4">
      {/* Top action header */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between pb-6 mb-8 border-b border-slate-205 dark:border-slate-800/40">
                  ...
                  {filtered.map((job) => {
                    ...
                    return (
                      <motion.div
                        ...
                        className="p-4 rounded-2xl bg-white dark:bg-dark-400 border border-slate-200/50 dark:border-slate-800/60 shadow-sm hover:shadow-md hover:border-brand-500/40 dark:hover:border-brand-500/30 transition text-left cursor-pointer relative group"
                      >
                        <h4 className="text-xs font-bold text-slate-900 dark:text-white truncate pr-6 group-hover:text-brand-605 dark:group-hover:text-brand-400">
                          {job.title}
                        </h4>
                        ...
                        <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity flex space-x-1">
                          {col.id !== "Interview" && col.id !== "Selected" && (
                            <button
                              ...
                              className="p-1 rounded bg-purple-500/10 text-purple-600 dark:text-purple-400 hover:bg-purple-500 hover:text-white transition"
        ...
        {isAddOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <h2 className="text-xl font-bold text-slate-905 dark:text-white flex items-center">
              ...
              <form onSubmit={handleAddJob} className="mt-6 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    >
                      <option value="Applied" className="text-slate-900">Applied</option>
                      <option value="Interview" className="text-slate-900">Interview</option>
                      <option value="Selected" className="text-slate-900">Selected</option>
                ...
                <div className="space-y-2 pt-2 border-t border-slate-100 dark:border-slate-800/40">
                  <label className="block text-[11px] font-bold text-slate-450 dark:text-slate-500 uppercase">Preparation Checklist</label>
                  ...
                  <div className="max-h-28 overflow-y-auto space-y-2 mt-2">
                    {newChecklist.map((item, idx) => (
                      <div key={idx} className="flex items-center justify-between bg-slate-50/50 dark:bg-slate-900/30 p-2 rounded-lg border border-slate-200/30 dark:border-slate-800/30">
                        <div className="flex items-center space-x-2">
                          <input
                            type="checkbox"
                            checked={item.completed}
                            onChange={() => toggleChecklistItem(false, idx)}
                            className="rounded border-slate-300 text-brand-605 focus:ring-brand-500"
                          />
                          <span className={`text-[11px] font-medium ${item.completed ? "line-through text-slate-400" : "text-slate-700 dark:text-slate-300"}`}>
                            {item.text}
                          </span>
                        </div>
                        <button
                ...
                <div className="flex justify-end space-x-3 pt-4 border-t border-slate-100 dark:border-slate-800/40">
                  <button
                    type="button"
                    onClick={() => setIsAddOpen(false)}
                    className="px-4.5 py-2.5 rounded-xl border border-slate-205 text-slate-600 dark:border-slate-800 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-900/40 text-xs font-bold transition"
                  >
                    Cancel
                  </button>
                  <button
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-between items-center pt-4 border-t border-slate-100 dark:border-slate-800/45">
                  <button
                    type="button"
              ...
              <div className="border-t md:border-t-0 md:border-l border-slate-200/40 dark:border-slate-800/40 pt-4 md:pt-0 md:pl-6 space-y-5 flex flex-col justify-between">
                <div>
                  <h3 className="text-xs font-extrabold uppercase tracking-wider text-slate-450 mb-3 flex items-center">
                  ...
                  <div className="max-h-36 overflow-y-auto space-y-2">
                    {editChecklist.map((item, idx) => (
                      <div key={idx} className="flex items-center justify-between p-2 rounded-lg bg-slate-50/50 dark:bg-slate-900/30 border border-slate-200/30 dark:border-slate-800/30">
                        <div className="flex items-center space-x-2 text-left">
                          <input
                            type="checkbox"
                            checked={item.completed}
                            onChange={() => toggleChecklistItem(true, idx)}
                            className="rounded border-slate-350 text-brand-605 focus:ring-brand-500"
                          />
                ...
                <div className="pt-4 border-t border-slate-100 dark:border-slate-800/45 space-y-3">
                  <div className="flex justify-between items-center">
                    ...
                    {!aiEmail && (
                      <button
                        ...
                        className="text-[10px] text-brand-650 dark:text-brand-400 font-bold hover:underline flex items-center"
                      >
                        {aiEmailLoading ? <Loader2 className="w-3 h-3 animate-spin mr-1" /> : <Mail className="w-3 h-3 mr-1 text-brand-500" />}
                  ...
                  {aiEmail && (
                    <div className="p-3.5 rounded-xl bg-brand-500/[0.02] border border-brand-500/10 space-y-2 relative">
                      <div className="flex justify-between items-center pb-1 border-b border-slate-100 dark:border-slate-800/40">
                        ...
                        >
                          {emailCopied ? <Check className="w-3 h-3 text-emerald-500 mr-0.5" /> : <Copy className="w-3 h-3 mr-0.5" />}
                          <span>{emailCopied ? "Copied" : "Copy"}</span>
                        </button>
                      </div>
                      <pre className="text-[10px] text-slate-650 dark:text-slate-300 leading-relaxed font-sans whitespace-pre-wrap max-h-36 overflow-y-auto">
                        {aiEmail}
                      </pre>
                    </div>
                  )}
